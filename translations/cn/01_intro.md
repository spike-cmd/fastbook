...
1.  什么是度量？它与损失有什么不同？
    `目标：损失用于训练过程中优化模型参数的目标函数。度量用于评估模型的性能。使用过程：损失主要在训练过程中使用，通过最小化损失函数来优化模型。度量在训练和验证过程中用来评估模型的好坏。反馈信号：损失提供模型参数调整的反馈信号。度量提供模型整体性能的评估信号`
1.  预训练模型有何帮助？
    `加速训练过程、提高模型能力、减少对大数据的依赖、迁移学习、解决特定领域问题`
1.  模型的“头”是什么？

1.  CNN 的早期层找到了什么样的特征？后期层呢？
    `早期：边缘、纹理。中间：形状和模式、局部特征。后期：高级语义特征`
1.  图像模型仅对照片有用吗？
